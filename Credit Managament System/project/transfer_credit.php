...
<body>
	<div class="heading"><img alt="Brand" src="http://localhost/project/logo.png" width="90" height="90" style="position: absolute;top:0%;"></div>

			<?php
			ini_set('error_reporting',E_ALL & ~E_NOTICE );
				$ID = $_GET['ID'];
				$result=mysqli_query($conn,"SELECT name,id,email,credit FROM users WHERE name='". $ID ."' ");
			?>
	<div class="data">
		<table class="table" style="position: absolute;left:5%;width:30%">
			<thead>
			<tr>
			<th style="text-align: center;font-size: 20px;">USER DETAILS</th>	
			</tr>
			</thead>
			<tbody>
				<?php
	...
	<div class="form">
		<h3>Transfer credits
		<img alt="Brand" src="http://localhost/project/logo.png" width="60" height="60">
	</h3>
		<form action="transfer_op.php" method="post">

			<label style="font-size:20px;">From</label>
			<?php
				echo "<input type=\"text\" name=\"from\" value=\"".$ID."\" required size=\"30\"  style=\"border-radius:5px\;padding:4px 15px\;border-width:1px\;border-color:black; width:140%;\" readonly>";
				?>
				<br><br><br>
			...
			<select name="status1" id="status">
				...
				<?php
  				...
  				<br><br>
  				<a href="http://localhost/project/transfer.php" style="color:black">Back to Home!!</a>
		</form>
	</div>
